...
namespace SistemaVenta.BLL.Implementacion
{
    public class VentaService : IVentaService
    {
        ...
        {
            // Aplicamos filtros de busqueda.
            IQueryable<Producto> query = await _repositorioProducto.Consultar( p => 
                p.EsActivo == true &&
                p.Stock > 0 &&
                string.Concat(p.CodigoBarra, p.Marca, p.Descripcion).Contains(busqueda) // Concatenamos columnas para aplicar filtro de busqueda
            );
        ...
        public async Task<List<Venta>> Historial(string numeroVenta, string fechaInicio, string fechaFin)
        {
            ...
            // Verificamos que lleguen dos fechas validas
